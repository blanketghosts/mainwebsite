...
function closeAllDropdowns() {
  ...
  document.querySelectorAll('.menu-item, .menu-logo').forEach(m => m.classList.remove('active'));
}
...
function handleDropdownAction(action) {
  closeAllDropdowns();
  switch (action) {
    case 'open-poetry':      openWindow('poetry');  break;
    case 'open-music':       openWindow('music');   break;
    case 'open-works':       openWindow('works');   break;
    case 'open-about':       openWindow('about');   break;
    case 'arrange-windows':  arrangeWindows();      break;
    case 'close-all':        closeAllWindows();     break;
  }
}
...
function showPoem(index) {
  ...
  document.querySelector('#window-poetry .status-left').textContent = poem.title;
}
...
function showPoetryList() {
  ...
  document.querySelector('#window-poetry .status-left').textContent = '5 poems';
}
...
function initPoetryWindow() {
  ...
  document.querySelectorAll('.list-row[data-poem]').forEach(row => {
    ...
    row.addEventListener('keydown', e => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); showPoem(idx); }
    });
  });
...
/* ============================================================
   10. WINDOW CLICK → FOCUS (mousedown on any part of window)
   ============================================================ */
...
/* ============================================================
   11. GLOBAL MOUSE & TOUCH EVENTS (drag + resize routing)
   ============================================================ */
...
function initGlobalEvents() {
  ...
  document.addEventListener('touchmove', e => {
    if (dragState) { e.preventDefault(); onDragMove(e); }
  }, { passive: false });
  document.addEventListener('touchend', onDragEnd);
}

/* ============================================================
   12. ATTACH TITLE-BAR & RESIZE HANDLE LISTENERS
   ============================================================ */
...
/* ============================================================
   13. RESPONSIVE: body.mobile class for CSS hooks
   ============================================================ */
